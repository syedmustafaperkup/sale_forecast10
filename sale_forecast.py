    ...
    def generate_forecast(self):
        ...
        for index,item in enumerate(periods):
            ...
            for product in product_ids:
                ...
                ctx.update({'from_date': item.name, 'to_date': end_date, 'warehouse': self.warehouse_id.id, 'current_model': 'sale.forecast', 'rec_id': self.id})
                product_qty = self.env['product.product'].browse(product.id)#_product_available(self._cr,1, [product.id], False, False, ctx)
